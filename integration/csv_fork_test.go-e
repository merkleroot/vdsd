...
import (
	...
	"github.com/merkleroot/vdsd/integration/rpctest"
...
func makeTestOutput(r *rpctest.Harness, t *testing.T,
	amt vdsutil.Amount) (*btcec.PrivateKey, *wire.OutPoint, []byte, error) {
	...
	// spend.
	a, err := vdsutil.NewAddressPubKey(key.PubKey().SerializeCompressed(), r.ActiveNet)
	if err != nil {
		return nil, nil, nil, err
	}
...
func TestBIP0113Activation(t *testing.T) {
	...
	vdsdCfg := []string{"--rejectnonstd"}
	r, err := rpctest.New(&chaincfg.SimNetParams, nil, vdsdCfg)
	if err != nil {
	...
	const outputValue = vdsutil.SatoshiPerBitcoin
...
func createCSVOutput(r *rpctest.Harness, t *testing.T,
	numSatoshis vdsutil.Amount, timeLock int32,
	...
	p2shAddr, err := vdsutil.NewAddressScriptHash(csvScript, r.ActiveNet)
	if err != nil {
		return nil, nil, nil, err
	}
...
func TestBIP0068AndBIP0112Activation(t *testing.T) {
	...
	const (
		outputAmt         = vdsutil.SatoshiPerBitcoin
