...
type memWallet struct {
	...
	addrs map[uint32]vdsutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]vdsutil.Address)
...
// SetRPCClient saves the passed rpc connection to vdsd as the wallet's
...
func (m *memWallet) IngestBlock(height int32, header *wire.BlockHeader, filteredTxns []*vdsutil.Tx) {
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash,
	...
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          vdsutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (vdsutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []vdsutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt vdsutil.Amount,
	feeRate vdsutil.Amount, change bool) error {
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := vdsutil.Amount(txSize * int(feeRate))
...
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut,
	...
	var outputAmt vdsutil.Amount
	for _, output := range outputs {
		outputAmt += vdsutil.Amount(output.Value)
		tx.AddTxOut(output)
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() vdsutil.Amount {
	m.RLock()
	defer m.RUnlock()

	var balance vdsutil.Amount
...
func keyToAddr(key *btcec.PrivateKey, net *chaincfg.Params) (vdsutil.Address, error) {
	serializedKey := key.PubKey().SerializeCompressed()
	pubKeyAddr, err := vdsutil.NewAddressPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
