...
import (
	...
	rpc "github.com/merkleroot/vdsd/rpcclient"
	"github.com/merkleroot/vdsutil"
)

// nodeConfig contains all the args, and data required to launch a vdsd process
...
func newConfig(prefix, certFile, keyFile string, extra []string) (*nodeConfig, error) {
	vdsdPath, err := btcdExecutablePath()
	if err != nil {
		vdsdPath = "ltcd"
	}
...
// arguments returns an array of arguments that be used to launch the vdsd
// process.
...
// command returns the exec.Cmd which will be used to start the vdsd process.
...
// to the vdsd process that is launched via Start().
...
// as the base for the log and data directories for vdsd.
...
// start creates a new vdsd process, and writes its pid in a file reserved for
...
// stop interrupts the running vdsd process process, and waits until it exits
...
// shutdown terminates the running vdsd process, and cleans up all
...
func genCertPair(certFile, keyFile string) error {
	...
	cert, key, err := vdsutil.NewTLSCertPair(org, validUntil, nil)
	if err != nil {
		return err
	}
