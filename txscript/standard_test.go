...
// newAddressPubKey returns a new vdsutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) vdsutil.Address {
	addr, err := vdsutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new vdsutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) vdsutil.Address {
	addr, err := vdsutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new vdsutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) vdsutil.Address {
	addr, err := vdsutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the vdsutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the vdsutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := vdsutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := vdsutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := vdsutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := vdsutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := vdsutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*vdsutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*vdsutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*vdsutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*vdsutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*vdsutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
