...
const (
	...
	MinHighPriority = vdsutil.SatoshiPerBitcoin * 144.0 / 250
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr vdsutil.Address) (*vdsutil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *vdsutil.Tx, height int32) error {
...
func logSkippedDeps(tx *vdsutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
//  |  <= policy.BlockMinSize)          |   |
//   -----------------------------------  --
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress vdsutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*vdsutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		case segwitActive && !witnessIncluded && tx.HasWitness():
			...
			// witness commitment.
			coinbaseCopy := vdsutil.NewTx(coinbaseTx.MsgTx().Copy())
	...
	block := vdsutil.NewBlock(&msgBlock)
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A vdsutil.Block should use saved in the state to avoid
