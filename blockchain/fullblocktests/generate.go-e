...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: vdsutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := vdsutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*vdsutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, vdsutil.NewTx(tx))
	}
...
func additionalCoinbase(amount vdsutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee vdsutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee vdsutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee vdsutil.Amount) *wire.MsgTx {
